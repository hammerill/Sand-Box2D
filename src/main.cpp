...
void objCreateCycle(WorldManager* wm, SDL_Texture* texture)
{
    while (true)
    {
        ...
        PObjBox* box = new PObjBox(texture, 7, 0, randFloat(min, max), randFloat(min, max), 0, 10, 10);
        box->isMarkedToDelete = true;
        wm->addObject(box);
        
        PObjCircle* circle = new PObjCircle(5, 0, randFloat(min, max) / 3.0, -10, 10);
...
{    
    ...
    wm->addObject(new PObjPlatform(2, 7, 10, 7)); // BOTTOM __ /

    wm->addObject(new PObjPlatform(1, 1, 2, 7)); // LEFT \ /
    wm->addObject(new PObjPlatform(10, 7, 11, 1)); // RIGHT / /
