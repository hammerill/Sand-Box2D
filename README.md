...
7. MainMenu for release state at future:
  ...
  - In game (when no fade anims are running) you can call PauseMenu. When you do that, physics stop and you can access these buttons: "RESUME" (close PauseMenu and return to game), "SETTINGS" (open OptionsMenu and keep game state) and "EXIT" (starts fadeout effect, during which we can see physics temporary resume. At the end returns us to MainMenu). `START` button with which we opened PauseMenu should also close it. Would be better if there's some way to blur background (render game scene to texture, blur it and then show it behind PauseMenu maybe?).
